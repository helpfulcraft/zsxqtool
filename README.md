...
1.  **抓取 (Crawl)**: 使用 `zsxq_crawler.py` 从知识星球 API 获取原始数据，并将其以结构化的方式存储为本地 Markdown 文件。
2.  **AI 处理 (AI Process)**: 使用 `process_with_ai.py` 调用大语言模型（LLM）对抓取的原始 Markdown 文件进行内容增强，如生成标签和摘要。
3.  **生成 HTML (Generate)**: 使用 `build_html.py` 将所有处理过的数据汇集起来，生成一个功能丰富的单页面静态网站。
